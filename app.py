...
# ================= Main Loop (with Streamlit-friendly sleep) =================
while True:
    ret, img = cap.read()
    if not ret:
    ...
    if hands:
        ...
        if dist < 45 and time.time()-last_click > 0.6:
            ...
            if click_key:
                playsound("click.wav")
                if click_key=="Space": text+=" "
                elif click_key=="Del": text=text[:-1]
                elif click_key=="Shift": caps=not caps
                elif click_key=="Save": pass
                else: text+=click_key if caps else click_key.lower()
    ...
    # ================= Update Sidebar =================
    text_box.markdown(f"<div style='background-color:black; padding:10px; color:white; height:400px; overflow:auto;'>{text}</div>", unsafe_allow_html=True)
